...
iRay  | Programming
	...
	function RayfieldLibrary:CreateWindow(Settings)
		...
		Main.Size = UDim2.new(0, 325, 0, 250) -- Dikdörtgen oran
		Main.Visible = true
		Main.BackgroundTransparency = 1
		LoadingFrame.Title.TextTransparency = 1
		LoadingFrame.Subtitle.Visible = false -- Subtitle'ı gizle
		Main.Shadow.Image.ImageTransparency = 1
		LoadingFrame.Version.Visible = false -- Version metnini gizle
		LoadingFrame.Title.Text = Settings.LoadingTitle or "Rayfield Interface Suite"

		if Settings.LoadingTitle ~= "Rayfield Interface Suite" then
			LoadingFrame.Version.Text = "Made By William Butcher"
		end
		Topbar.Visible = false
		Elements.Visible = false
		LoadingFrame.Visible = true

		-- Metinler ve renkler için tablolar
		local texts = {
		...
		-- Rastgele metin ve renk seçimi
		local randomText = texts[math.random(1, #texts)]
		local randomColor = colors[math.random(1, #colors)]

		-- GIF gibi görüntüler dizisi
		local frames = {
			"rbxassetid://136200282848457", -- İlk kare
			"rbxassetid://112564686108512", -- İkinci kare
		}

		-- ImageLabel oluştur ve ayarlarını yap
		local ImageLabel = Instance.new("ImageLabel")
		ImageLabel.Parent = LoadingFrame
		ImageLabel.Name = "eleman"
		ImageLabel.AnchorPoint = Vector2.new(0.5, 0.5) -- Ortalamak için anchorpoint ayarla
		ImageLabel.Position = UDim2.new(0.5, 0, 0.5, 0) -- Ortaya konumlandır
		ImageLabel.Size = UDim2.new(0, 150, 0, 150) -- Boyutunu sabit bırak
		ImageLabel.BackgroundTransparency = 1
		ImageLabel.ImageTransparency = 1
		ImageLabel.Visible = true

		-- Title'ı ImageLabel'in X eksenine göre ortalamak için ayarla
		LoadingFrame.Title.AnchorPoint = Vector2.new(0.5, 0) -- Ortalamak için anchorpoint ayarla
		LoadingFrame.Title.Position = UDim2.new(ImageLabel.Position.X.Scale, ImageLabel.Position.X.Offset, 0.1, 0) -- X eksenine göre ortala
		LoadingFrame.Title.TextXAlignment = Enum.TextXAlignment.Center -- Ortadan genişlesin

		-- ClonedTitle oluştur ve ImageLabel'in X eksenine göre ortala
		local ClonedTitle = LoadingFrame.Title:Clone()
		ClonedTitle.Parent = LoadingFrame
		ClonedTitle.AnchorPoint = Vector2.new(0.5, 1) -- Ortalamak için anchorpoint ayarla
		ClonedTitle.Position = UDim2.new(ImageLabel.Position.X.Scale, ImageLabel.Position.X.Offset, 0.9, 0) -- X eksenine göre ortala ve biraz aşağıda
		ClonedTitle.TextTransparency = 1
		ClonedTitle.Text = randomText
		ClonedTitle.TextColor3 = randomColor
		ClonedTitle.TextXAlignment = Enum.TextXAlignment.Center -- Ortadan genişlesin

		-- GIF animasyonunu başlatan fonksiyon
		local function playGif()
			...
			while true do
				...
				wait(0.4) -- Kadranlar arasında 0.1 saniye bekle
			end
		end
		...
		-- LoadingFrame'i görünür hale getirdiğimizde ImageLabel'i de görünür yapıyoruz
		LoadingFrame.Visible = true
		ImageLabel.Visible = true    
		...
		wait(0.1)
		TweenService:Create(ImageLabel, TweenInfo.new(0.7, Enum.EasingStyle.Quint), {ImageTransparency = 0}):Play() -- ImageLabel için görünme animasyonu
		TweenService:Create(LoadingFrame.Title, TweenInfo.new(0.7, Enum.EasingStyle.Quint), {TextTransparency = 0}):Play()
		TweenService:Create(ClonedTitle, TweenInfo.new(0.7, Enum.EasingStyle.Quint), {TextTransparency = 0}):Play()
		wait(0.05)
		...
		function Window:CreateTab(Name,Image)
			...
			function Tab:CreateButton(ButtonSettings)
				...
				Button.Interact.MouseButton1Click:Connect(function()
					-- Eğer isPremium aktifse ve geçersiz bir anahtar varsa, kullanımı engelle
					if ButtonSettings.isPremium and not isValidKey(script_key) then
						RayfieldLibrary:Notify({
							Title = "Hey!",
							Content = "You need a valid premium key to use this feature!",
							Duration = 7,
							Image = 15048277316
						})

						-- Kırmızı renge dön ve geri bildirim ver
						TweenService:Create(Button, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = Color3.fromRGB(255, 0, 0)}):Play()
						...
						return -- Kullanımı engelle
					end

					-- Geçerli anahtara sahip kullanıcılar veya isPremium olmayan kullanıcılar devam eder
			...
			function Tab:CreateInput(InputSettings)
				...
				Input.InputFrame.InputBox.FocusLost:Connect(function()
					...
					-- Eğer isPremium aktifse ve geçersiz bir anahtar varsa, işlemi engelle
					if InputSettings.isPremium and not isValidKey(script_key) then
						RayfieldLibrary:Notify({
							Title = "Hey!",
							Content = "You need a valid premium key to use this feature!",
							Duration = 7,
						...
						-- Kırmızı renge dön ve geri bildirim ver
						TweenService:Create(Input, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = Color3.fromRGB(255, 0, 0)}):Play()
			...
			function Tab:CreateDropdown(DropdownSettings)
				...
				table.sort(DropdownSettings.Options, function(a, b)
					-- Öncelikle a ve b’nin sayısal olup olmadığını kontrol ediyoruz
					local aIsNumber = tonumber(string.sub(a, 1, 1)) ~= nil
					local bIsNumber = tonumber(string.sub(b, 1, 1)) ~= nil

					-- Harf ile başlayanlar, sayılarla başlayanların üstünde olmalı
					if aIsNumber and not bIsNumber then
						return false
					elseif not aIsNumber and bIsNumber then
						return true
					end

					-- İkisi de aynı türdeyse, alfabetik ya da sayısal sıralama yap
					return a < b
				end)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						-- Eğer isPremium aktifse ve geçersiz bir anahtar varsa, işlemi engelle
						if DropdownSettings.isPremium and not isValidKey(script_key) then
							RayfieldLibrary:Notify({
								Title = "Hey!",
								Content = "You need a valid premium key to use this feature!",
								Duration = 7,
							...
							-- Kırmızı renge dön ve geri bildirim ver
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = Color3.fromRGB(255, 0, 0)}):Play()
			...
			function Tab:CreateToggle(ToggleSettings)
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					-- Eğer isPremium aktifse ve geçersiz bir anahtar varsa, kullanımı engelle
					if ToggleSettings.isPremium and not isValidKey(script_key) then
						RayfieldLibrary:Notify({
							Title = "Hey!",
							Content = "You need a valid premium key to use this feature!",
							Duration = 7,
						...
						-- Kırmızı renge dön ve CurrentValue'u false yap
						ToggleSettings.CurrentValue = false
						TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = Color3.fromRGB(255, 0, 0)}):Play()
						...
						return -- Toggle çalışmasını durdurur
					end

					-- isPremium olmayan kullanıcılar veya geçerli bir anahtara sahip premium kullanıcılar buradan devam eder
		...
		TweenService:Create(ImageLabel, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {ImageTransparency = 1}):Play() -- ImageLabel için eklenen animasyon
...
local cg = game:GetService("CoreGui"):GetChildren()

for _, child in ipairs(cg) do
